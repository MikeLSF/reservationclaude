...
export function Calendar({
  ...
  // Update currentMonth when availableDates change
  // This ensures the calendar updates when the parent component fetches new data for a new month
  useEffect(() => {
    if (availableDates.length > 0) {
      // Get the month of the first available date
      ...
      // If the month of the first available date is different from the current month,
      // update the current month
      if (firstDateMonth !== currentMonth.getMonth() || firstDateYear !== currentMonth.getFullYear()) {
        console.log("Available dates month changed, updating current month");
        setCurrentMonth(new Date(firstDateYear, firstDateMonth, 1));
      }
    }
  ...
  const nextMonth = () => {
    ...
    if (onMonthChange) {
      ...
      // Update the local state as well to ensure the UI updates immediately
      setCurrentMonth(newMonth);
    } else {
      // If no onMonthChange is provided, just update the local state
      setCurrentMonth(newMonth);
    }
  };
